...
import { testMongodb, Post, Notification } from '../src/mongo';
...
describe('when deleting posts', () => {
  ...
  test('all notifications related to post are deleted', async () => {
    const post = (await testHelpers.postsInDB())[0];

    const testUser2 = await testHelpers.createTestUser({
      username: 'bobbybo2',
      fullName: 'Bobby Bo2',
