...
import { testMongodb, User, Notification } from '../src/mongo';
...
describe('notifications', () => {
  ...
  beforeEach(async () => {
    ...
    [testUser] = (await testHelpers.usersInDB());
  ...
  test('following another user creates a notification for the followed user', async () => {
    const users = await testHelpers.usersInDB();
    const differentUser = users.find((user) => user.id !== testUser.id);
  ...
  test('following another user emits a notification to the followed user', async () => {
    const users = await testHelpers.usersInDB();
    const differentUser = users.find((user) => user.id !== testUser.id);
    ...
    expect(emitNotificationSpy).toHaveBeenCalledWith(differentUser.id.toString(), 'follow');
  });

  test('user following themselves does not create a notification', async () => {
    await api
      .put(`/api/users/${testUser.id}/follow`)
      .set('Authorization', `bearer ${accessToken}`)
      .expect(400);
  ...
  test('unfollowing a user removes the notification for the followed user', async () => {
    const users = await testHelpers.usersInDB();
    const differentUser = users.find((user) => user.id !== testUser.id);
    ...
    expect(notificationBefore).toBeDefined();

    await api
      .put(`/api/users/${differentUser.id}/unfollow`)
      .set('Authorization', `bearer ${accessToken}`)
      .expect(200);
